...
public class RodsUtility implements CloudOperations {
		...
		public void login() throws CloudException {
			...
			buildHomePath();
			account = new IRODSAccount(host, port, user, password, homePath, zone, res);
			try {
	    ...
	    public void addChildrenFolder(DefaultMutableTreeNode node, DefaultTreeModel Treemodel, String name) throws CloudException {
	    	IRODSFile irodsFile;
	    	irodsFile = accessFile(name);
			...
			for (File child : children) {
				if (child.isDirectory()) {
					DefaultMutableTreeNode nodeChild = new DefaultMutableTreeNode(child.getName());
					GeneralUtility.addUniqueNode(node, nodeChild, Treemodel);
				}
			}
	    }
